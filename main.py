...
if __name__ == '__main__':
  ...
  if args.mode == 'test':
    ...
    acciones = { 0:'sell', 1:'hold', 2:'buy'}
    labels_added = {0: False, 1: False, 2: False}

    print('valor final_portafolio',val[-1])
    for i in range(len(test_data)):
      if i == 0:
        plt.plot(i,test_data[i],color=color_dict[1],marker='o')
      else:
        if not labels_added[actions_record[i-1]]:
          labels_added[actions_record[i-1]] = True
          plt.plot(i,test_data[i],color=color_dict[actions_record[i-1]],marker='o',label=acciones[actions_record[i-1]])
        else:
